...
class TestSOCKS5Agent(AgentTestCase):
    ...
    def test_HTTPRequest(self):
        ...
        self.assertEqual(received[:18], six.b('\x05\x01\x00\x05\x01\x00\x03\x08spam.com\x00\x50'))
        request = received[18:].splitlines()
        self.assert_(six.b('GET /eggs HTTP/1.1') in request)
        self.assert_(six.b('Host: spam.com') in request)

    def test_HTTPSRequest(self):
        self.agent.request('GET', 'https://spam.com/eggs')
        self.endpoint.proto.dataReceived('\x05\x00\x05\x00\x00\x01444422')
        received = self.endpoint.transport.value()
        self.assertEqual(received[:18], six.b('\x05\x01\x00\x05\x01\x00\x03\x08spam.com\x01\xbb'))
        request = received[18:].splitlines()
        if six.PY2:
            self.assert_(six.b('GET /EGGS HTTP/1.1') in request)
            self.assert_(six.b('HOST: SPAM.COM') in request)
        elif six.PY3:
            # PY3KPORT: There seems no uppercasing in HTTPS transport           
            self.assert_(six.b('GET /eggs HTTP/1.1') in request)
            self.assert_(six.b('Host: spam.com') in request)            
...
class TestSOCKS4Agent(AgentTestCase):
    ...
    def test_HTTP4Request(self):
        ...
        received = self.endpoint.transport.value()
        self.assertEqual(received[:9], six.b('\x04\x01\x00\x50\x7f\x00\x00\x01\x00'))
        request = received[9:].splitlines()
        self.assert_(six.b('GET /eggs HTTP/1.1') in request)
        self.assert_(six.b('Host: 127.0.0.1') in request)

    def test_HTTP4aRequest(self):
        self.agent.request('GET', 'http://spam.com/eggs')
        self.endpoint.proto.dataReceived('\x00\x5a\x00\x00\x00\x00\x00\x00')
        received = self.endpoint.transport.value()
        self.assertEqual(received[:18], six.b('\x04\x01\x00\x50\x00\x00\x00\x01\x00spam.com\x00'))
        request = received[18:].splitlines()
        self.assert_(six.b('GET /eggs HTTP/1.1') in request)
        self.assert_(six.b('Host: spam.com') in request)
    ...
    def test_HTTPSRequest(self):
        ...
        self.endpoint.proto.dataReceived('\x00\x5a\x00\x00\x00\x00\x00\x00')
        received = self.endpoint.transport.value()
        self.assertEqual(received[:18], six.b('\x04\x01\x01\xbb\x00\x00\x00\x01\x00spam.com\x00'))
        request = received[18:].splitlines()
        if six.PY2:
            self.assert_(six.b('GET /EGGS HTTP/1.1') in request)
            self.assert_(six.b('HOST: SPAM.COM') in request)
